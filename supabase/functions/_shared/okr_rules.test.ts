...
// --- KR2: распознавание baseline→target в тексте ---

Deno.test("BASE_RULES: KR2 явно описывает распознавание baseline→target внутри текста KR", () => {
  const kr2Line = OKR_RULES_BLOCK.split("\n").find((l) => l.trim().startsWith("- KR2")) ?? "";
  assert(kr2Line.length > 0, "Строка KR2 должна присутствовать");
  assert(
    kr2Line.includes("с X до Y") && kr2Line.includes("baseline→target в самом тексте"),
    `KR2 должна явно описывать распознавание чисел в тексте. Получили: ${kr2Line}`,
  );
});

Deno.test("OKR_RULES_BLOCK_QUARTER унаследовал расширенную формулировку KR2", () => {
  assert(
    OKR_RULES_BLOCK_QUARTER.includes("baseline→target в самом тексте"),
    "Квартальный блок должен содержать расширенную формулировку KR2",
  );
});
